...
class BikeShifterNet(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(5, 64)  # 5 input features (excluding 'grades')
...
# Training_ML function
def Training_ML(powers, cadences, speeds, labels, timelastshift, min_accuracy=97):
    # Convert labels to numeric if they are not
    if isinstance(labels[0], str):
        encoder = LabelEncoder()
        print(labels)
        # Check if labels is not empty before applying fit_transform
        if labels:
            labels = encoder.fit_transform(labels)
        else:
            # Handle the case when labels is empty
            print("No labels found. Returning None.")
            return None, None

    # Combine features and preprocess
    # Make sure all lists are of the same length and aligned
    features = list(zip(powers, cadences, speeds, labels, timelastshift))
    ...
    features_train, features_test, labels_train, labels_test = train_test_split(features, labels, test_size=.9,
                                                                                random_state=42)
